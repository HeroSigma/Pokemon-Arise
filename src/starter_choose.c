...
static void Task_TransitionToStarterSelection(u8 taskId);
...
static void Task_TransitionToStarterSelection(u8 taskId)
{
    ...
    // Create new Poké Ball sprites for starter selection
    for (i = 0; i < STARTER_MON_COUNT; i++)
    {
        spriteId = CreateSprite(&sSpriteTemplate_Pokeball, sPokeballCoords[i][0], sPokeballCoords[i][1], 2);
        gSprites[spriteId].sTaskId = taskId;
    ...
    // Reset selection to middle starter and switch to phase 2
    gTasks[taskId].tStarterSelection = 1;
    gTasks[taskId].tPhase = 1;
    gTasks[taskId].func = Task_HandleStarterSelectionInput;
    
    // Create initial label for phase 2
    CreateStarterPokemonLabel(1);
}
