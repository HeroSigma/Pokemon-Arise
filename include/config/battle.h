...
#define B_SMART_WILD_AI_FLAG        FLAG_TEMP_5     // If this flag is set, wild Pokémon will become smart, with all AI flags enabled.
...
#define B_FLAG_DYNAMAX_BATTLE       FLAG_TEMP_6     // If this flag is set, the ability to Dynamax in battle is enabled for all trainers.
#define B_FLAG_TERA_ORB_CHARGED     FLAG_TEMP_7     // If this flag is set, the Tera Orb is charged. It is automatically set upon healing and cleared upon Terastallizing once configured.
...
#define B_FLAG_SLEEP_CLAUSE         FLAG_TEMP_8     // If this flag is set, sleep clause is enabled; if the player / AI has already put a Pokémon on the opponent's side to sleep and it is still sleeping, another one can't be put to sleep. AI requires AI_FLAG_CHECK_BAD_MOVE to understand.
#define B_FLAG_NO_WHITEOUT          FLAG_TEMP_9     // If this flag is set, the player can not white out against Trainers. Please note that the party is not healed automatically!
...
#define B_VAR_DIFFICULTY            VAR_UNUSED_0x4091     // If not 0, you can use this var to control which difficulty version of a Trainer is loaded. This should be manually set by developer using Script_SetDifficulty AFTER NewGameInitData has run.
